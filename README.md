...
- **Write verification**: because the write itself goes over a `no-cors` POST (a constraint of Apps Script web apps, not something this app can avoid), the browser can't read whether that specific request succeeded. To compensate, after every submission the app polls the sheet back over a normal, readable request and only shows "Saved" once it can see the new data there. If it can't confirm the write within a few seconds, it tells you plainly instead of showing a false success — and points you to the Apps Script editor's **Executions** panel, which is the one place that shows the actual server-side error if a `doPost` run fails.
